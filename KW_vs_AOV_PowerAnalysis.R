...
B <- 10000 #Number of Sims per Nseq
...
n3 = N/3
B = 10000
effectsizeseq <- seq(from = 0.5, to = 7, by = 0.25)
var <- 4
alpha <- 0.05
startmean <- 5
offset <- 0.5 #1 will give equal effect in both directions

kpp <- numeric(length(effectsizeseq))
app <- numeric(length(effectsizeseq))

for(j in 1:length(effectsizeseq)){
  g1mean <- startmean + effectsizeseq[j]
  g2mean <- startmean - ((offset)*effectsizeseq[j])
  g3mean <- startmean
  
  kp <- numeric(B)
  ap <- numeric(B)
  for (i in 1:B){
    p3df <- data.frame(group = factor(c(rep("g1", n1), rep("g2", n2), rep("g3", n3))),
                       val = c(rnorm(n1, g1mean, var), rnorm(n2, g2mean, var), rnorm(n3, g3mean, var))
    )
    
    test <- kruskal.test(x = p3df$val, g = p3df$group)
    kp[i] <-test$p.value
    
    ap[i] <- anova(lm(val ~ group, data = p3df))$'Pr(>F)'[1]
  }
  app[j] <- mean(ap < alpha)
  kpp[j] <- mean(kp < alpha)
}

df5 <- data.frame(Effect_Size = effectsizeseq, Kruskal = kpp, Anova = app)

#Plot the data
df_long5 <- melt(df5, id.vars = "Effect_Size", variable.name = "Test", value.name = "Power")

ggplot(df_long5, aes(x = Effect_Size, y = Power, color = Test)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
...
############# Now repeat parts 1:3 with a different distribution than normal####
################################################################################
...
for (j in 1:length(Nseq)){
  ...
  for (i in 1:B){
    p1df <- data.frame(group = c(rep('g1', times = n1), rep('g2', times = n2),
                                 rep('g3', times = n3)),
                       val = c(rcauchy(n1, g1mean, var), rcauchy(n2, g2mean, var),
                               rcauchy(n3, g3mean, var)))
...
simulate_imbalance <- function(group_means, var, B, alpha, imbalance_vals, fixed_n, fix = "n3") {
  ...
  for (k in imbalance_vals) {
    ...
    for (i in 1:B) {
      df <- data.frame(
        group = factor(c(rep("g1", n1), rep("g2", n2), rep("g3", n3))),
        val = c(rcauchy(n1, g1mean, var), rcauchy(n2, g2mean, var), rcauchy(n3, g3mean, var))
      )
